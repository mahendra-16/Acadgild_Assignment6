## Question-1 : Write a R program using control operators to test whether following values are
# prime numbers or not by providing a PRIME or NOT PRIME message as output :
...
## Question - 2 : Write a R program using control operators to identify letter u and a both occur in the
#following words:
...
## Question-3: Write a function that to calculate BMI (Body Mass Index):
...
## Question-4: Write a function called sum_of_cubes, that calculates the sum of cubes of the first n
# natural numbers :
...
## Question - 5: Write a function to calculate the mode (highest frequency) of the following vector:
...
## Question - 6: Write a function to calculate the no. of prime numbers of the following vector :
...
boys <- read.csv("boystop100s.csv", header = TRUE)
head(boys)
girls <- read.csv("girlstop100s.csv", header = TRUE)
head(girls)
...
# Question-9:Create R functions for the following operations.
